...
def load_pca_para(PCA_para_folder):
    pca_components = np.fromfile(os.path.join(PCA_para_folder,"pca_components_(3,9830400)"),dtype='float32').reshape((3, -1))
...
if __name__ == '__main__':
    ...
    # 加载CT
    CT_folder = os.path.join(root_path,"Dataset/Origin/CT")
    ...
    # file_save(pca_class.components_,'pca_components_(3,9830400)',os.path.join(root_path,"Dataset/Test_9dvf/DVF_trans_PCAs"))
    # file_save(pca_class.mean_, 'pca_mean_(9830400,)', os.path.join(root_path,"Dataset/Test_9dvf/DVF_trans_PCAs"))
    # file_save(ct_trans_pcas, "pca_9_phase", os.path.join(root_path,"Dataset/Test_9dvf/DVF_trans_PCAs"))
    #
    ...
    #         file_save(pca_trans_cts, ("DVF"+str(i)+'_'+str(j)), os.path.join(root_path,"Dataset/Test_9dvf/DVFs"))   # 保存对应的DVF
    #         del pca_trans_cts     # 减少内存的使用
    #         print(str(i)+'_'+str(j), '已经保存')
    #     gc.collect()   # 垃圾回收机制
    #
    # # 保存总的PCA
    # file_save(pca_all, ("pca_"+str((i+1) * linspace_num)), os.path.join(root_path,"Dataset/Test_9dvf/DVF_trans_PCAs"))   # 保存对应的pca
