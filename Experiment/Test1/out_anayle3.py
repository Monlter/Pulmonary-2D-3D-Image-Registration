...
def pd_save_csv(log_list):
    ...
    train_frame.to_csv(os.path.join(out_dir, "out_train1.csv"), sep=",", index=True, header=True)

    val_frame = pd.DataFrame(log_val_data)
    val_frame.to_csv(os.path.join(out_dir, "out_val1.csv"), sep=",", index=True, header=True)
