...
from tools.data_loader import Dataset_variable_TestNum
...
def val(Dataset_loader, net, loss_function, device):
    val_loss = 0
    with torch.no_grad():
        for i, (preImg,img, target) in enumerate(Dataset_loader):
            preImg = preImg.to(device)
            img = img.to(device)
            target = target.to(device)
            prediction = net(preImg,img)
...
def train(modelMethodName=args.common_model_name, dataMethodName=args.common_data_name,
    ...
    # 模型方式
    model_methods = {
        "CRN": LSTM_model.CRN(in_channels),
        "CRN_resnet":LSTM_model.CRN_resnet(in_channels)
    ...
    dataset = Dataset_variable_TestNum(img_folder, target_folder, PCA_all_folder, dataMethodName, 2)
    ...
    for epoch in range(args.EPOCH):
        loss_mse = 0
        for i, (preImg, img, target) in enumerate(train_data_loader):
            preImg = preImg.to(device)
            img = img.to(device)
            target = target.to(device)
            prediction = model(preImg,img)
...
if __name__ == '__main__':
    train(modelMethodName="CRN_resnet")
