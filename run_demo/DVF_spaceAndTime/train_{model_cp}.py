...
def val(Dataset_loader, reference_CBCT, net, STN_model, loss_function, device):
    val_loss = 0
    MAD = 0
    NCC = 0
    SSIM = 0
    with torch.no_grad():
        for i, (imgs, target) in enumerate(Dataset_loader):
            ...
            prediction_DVF = net(imgs)
            prediction_CBCT = STN_model(reference_CBCT, prediction_DVF).squeeze(dim=1)
            loss = loss_function(prediction_CBCT, target)
            val_loss += loss
            MAD += estimate_methods.MAE(target[0].cpu().detach().numpy().transpose(1, 2, 0),
                                        prediction_CBCT[0].cpu().detach().numpy().transpose(1, 2, 0))
            NCC += estimate_methods.NCC(target[0].cpu().detach().numpy().transpose(1, 2, 0),
                                        prediction_CBCT[0].cpu().detach().numpy().transpose(1, 2, 0))
            SSIM += estimate_methods.SSIM(target[0].cpu().detach().numpy().transpose(1, 2, 0),
                                          prediction_CBCT[0].cpu().detach().numpy().transpose(1, 2, 0))
...
def save_split(data, save_path, epoch):
    if len(data.shape) == 3:
        # CT_dcm shape= C,H,W
        ct_crossview = data[82, :, :]
        ct_coronalview = data[:, 76, :]
        ct_sagittalview = data[:, :, 30]
...
def train(modelMethodName=args.modelMethod, inputModeName=args.inputMode,
    ...
    # 模型方式
    model_methods = {
        "ConvLSTM_3D": convLSTM_3D.ConvLSTM_DVF(in_channels),
    }
    # 损失函数方式
    ...
    STN_model = STN.SpatialTransformer(vol_size).to(device)
    ...
    reference_CBCT = torch.tensor(load_odd_file(os.path.join(root_path, args.reference_CBCT)).reshape(150, 256, 256)[
                                  np.newaxis, np.newaxis, :, 53:203, 53:203].repeat(batch_size, axis=0)).to(device)
    # 训练参数设定
    logger.info(
    ...
    for epoch in range(args.EPOCH):
        ...
        for i, (imgs, target) in enumerate(train_data_loader):
            imgs = imgs.to(device)
            target = target.to(device)
            prediction_DVF = model(imgs)
            prediction_CBCT = STN_model(reference_CBCT, prediction_DVF).squeeze(dim=1)
            loss_item = loss_fn(target, prediction_CBCT)
            loss_mse += loss_item
        ...
        val_loss, MAD, NCC, SSIM = val(test_data_loader, reference_CBCT, model, STN_model, loss_fn, device)
        ...
        save_split(prediction_CBCT[0, ...].cpu().detach().numpy(), ct_split_dir, epoch + 1)
...
if __name__ == '__main__':
    train(modelMethodName="ConvLSTM_3D")
