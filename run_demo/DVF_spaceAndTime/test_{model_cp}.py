...
def exam_test(modelMethodName=args.modelMethod, inputModeName=args.inputMode,lossFunctionMethodName=args.lossFunctionMethod):
    ...
    model_methods = {
        "ConvLSTM": convLSTM_2D.ConvLSTM_Liner(in_channels),
    }
    ...
    for val_name in val_files:
        ...
        input_cur_img = data_processing.input_mode_concat_variable(cur_img, standardization_method="max_min",
                                                                   input_mode_names=inputModeName,
                                                                   resize=(120, 120))
        pre_imgs = data_processing.load_projection_sequence(val_img_folder, data_processing.get_preImgName_sequence(val_name, args.preImg_num))
        input_pre_imgs = [
            data_processing.input_mode_concat_variable(pre_img, standardization_method="max_min", input_mode_names=inputModeName,
